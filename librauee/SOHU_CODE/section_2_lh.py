...
def features_extractor():
    ...
    test = pd.read_csv('Sohu2022_data/test-dataset.csv')
    print(train.shape,test.shape)
    print(train.head())
    test['label'] = -2
    trainb_path= 'Sohu2022_data/rec-train-dataset.csv'
    testb_path = 'Sohu2022_data/rec-test-dataset.csv'
    ...
    data.to_feather('data/rec_data.feather')




def features_extractor2():
    data = pd.read_feather('data/rec_data.feather')
...
def features_extractor3():
    ...
    ## 加入历史序列情感
    res_emo = pd.read_csv('data/res(1).csv')
...
def features_extractor5():
    ...
    data = []
    f = open('Sohu2022_data/recommend_content_entity_0317.txt',encoding='utf-8',mode='r')
    for line in tqdm(f):
    ...
    f = open('Sohu2022_data/测试/recommend_content_entity.txt',encoding='utf-8',mode='r')
    for line in tqdm(f):
        line = json.loads(line)
        id = line['id']
        entity = ",".join(line['entity'])
    ...
    f = open('Sohu2022_data/训练/recommend_content_entity.txt',encoding='utf-8',mode='r')
    for line in tqdm(f):
        line = json.loads(line)
        id = line['id']
        entity = ",".join(line['entity'])
...
if __name__ == '__main__':
    ...
    oof,pred = mt.cat_test(cat_params=cat_params,)
    ...
    sub[['Id','result']].to_csv(f'section2_lh.txt',sep='\t',index=False)
