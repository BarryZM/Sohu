...
class CFG:
    apex=True
    num_workers=8
    model="hfl/chinese-pert-large"    # huggingface 预训练模型,可以访问 https://huggingface.co/models 选择更多其他模型
    ...
    encoder_lr=1e-5                      # 预训练模型内部参数的学习率
    decoder_lr=1e-5                      # 自定义输出层的学习率
    batch_size=4
    max_len=512
    weight_decay=0.01
    ...
    seed=2022
    n_fold= 5                       # 总共划分数据的份数
    trn_fold=[0,1,2,3, 4]                   # 需要训练的折数，比如一共划分了4份，则可以对应训练4个模型，1代表用编号为1的折做验证，其余折做训练
    train=False
...
############ test
import pandas as pd
test = pd.read_json('Sohu2022_data/测试/recommend_content_entity.txt', lines=True)
test_train2 = pd.read_json('Sohu2022_data/recommend_content_entity_0317.txt', lines=True)
test_train1 = pd.read_json('Sohu2022_data/训练/recommend_content_entity.txt', lines=True)
test = pd.concat([test, test_train1, test_train2], axis=0, sort=False).reset_index(drop=True)
...
res['pred'] = list(val_pred)
res.to_csv('data/res(1).csv', index=False)
